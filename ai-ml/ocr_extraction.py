...
def _fix_ocr_errors(text):
    ...
    text = re.sub(r'S\s+O\b', 'S/O', text)
    text = re.sub(r'D\s+O\b', 'D/O', text)
    text = re.sub(r'W\s+O\b', 'W/O', text)
    text = re.sub(r'C\s+O\b', 'C/O', text)
...
def _extract_address(all_items, mid_x=None, img_w=None):
    ...
    address_text = re.sub(r'S\s+O\b', 'S/O', address_text)
    address_text = re.sub(r'D\s+O\b', 'D/O', address_text)
    address_text = re.sub(r'W\s+O\b', 'W/O', address_text)
    address_text = re.sub(r'C\s+O\b', 'C/O', address_text)
    ...
    address_text = re.sub(r'(Coimbatore|Chennai|Bangalore|Mumbai|Delhi|Pune|Hyderabad|Tamil\s*Nadu|Karnataka|Kerala|Maharashtra|Andhra\s*Pradesh|Telangana)\s+(?=[A-Za-z])', r'\1, ', address_text, flags=re.IGNORECASE)
